...
public class BoletosApiController {
    ...
    @GetMapping("/{compraId}")
    public ResponseEntity<ApiResponse<Compra>> verBoletos(@PathVariable Integer compraId, HttpSession session) {
        ...
        if (!compra.getCliente().getId().equals(usuario.getId())) throw new BusinessException("No autorizado para ver esta compra");
